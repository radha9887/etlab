interface GcsSensorConfigProps {
  config: {
    taskId: string;
    bucket?: string;
    object?: string;
  ...
  onChange: (updates: Partial<GcsSensorConfigProps['config']>) => void;
  etlPages: any[];
}

export const GcsSensorConfig = ({ config, onChange }: GcsSensorConfigProps) => {
  return (
    <div className="space-y-3">
      ...
      {/* Object / Prefix */}
      <div>
        <label className="block text-xs text-gray-400 mb-1">Object Path or Prefix *</label>
        <input
          type="text"
          value={config.object || config.prefix || ''}
        ...
        <p className="text-xs text-gray-500 mt-1">
          Full object path or prefix to monitor
        </p>
      </div>
      ...
      <div>
        ...
        />
        <p className="text-xs text-gray-500 mt-1">
          Airflow connection for GCP credentials
        </p>
      </div>
      ...
      {config.bucket && config.object && (
        <div className="bg-blue-500/10 border border-blue-500/30 rounded-md p-2">
          <p className="text-xs text-blue-400 font-mono">
            gs://{config.bucket}/{config.object}
          </p>
        </div>
      )}
    </div>
